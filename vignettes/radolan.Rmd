...
`opendata_files` that is available in the package. You can browse through this
data frame to find the files that you are looking for:

```{r eval = TRUE}
# Get data frame of all files
dwd_files <- kwb.dwd::opendata_files

# Filter for historical RADOLAN files
radolan_files <- dwd_files[grep("radolan/historical", dwd_files$path), ]

# Shorten the path for better display
radolan_files$path <- gsub(ftp_root, "<root>/", radolan_files$path)
